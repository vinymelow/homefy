...
├── dashboard/
│   └── server.py                 ← painel web local (python3 dashboard/server.py)
├── config/
...
## Dashboard web (opcional, sem dependências)

```bash
python3 dashboard/server.py          # http://IP_DO_VPS:8000/?key=CHAVE
nohup python3 dashboard/server.py &  # em background
```

Painel de gestão: estado da ligação Shopify, lista de briefs, wizard de criação de
brief e plano de teste. A chave de acesso vai em `DASHBOARD_KEY=` no `config/.env`
(se não estiver definida, é gerada e impressa no arranque — guarda-a). Guia completo
do dia a dia: `docs/guia-uso.md`.
